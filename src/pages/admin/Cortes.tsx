...
// La estructura real del backend: { corte: {...}, contrato: {...} }
interface CorteRow {
    corte: {
        id: string
        contratoId: string
...
const Cortes = () => {
    ...
    const fetchCortes = useCallback(async () => {
        ...
        try {
            ...
            // El backend devuelve array de { corte, contrato }
                        ...
                        {loadingChart ? (
                            ...
                            <ResponsiveContainer width="100%" height={200}>
                                <BarChart data={porDistrito} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                                    ...
                                    <XAxis dataKey="distritoNombre" tick={{ fontSize: 11, fill: '#94a3b8' }} tickLine={false} axisLine={false} />
                            ...
                            <tbody>
                                {filtered.map((r, i) => (
                                    <tr key={r.corte.id} className={`border-b border-slate-50 hover:bg-slate-50/60 transition-colors ${i % 2 === 0 ? '' : 'bg-slate-50/30'}`}>
                                        ...
                                        <td className="px-4 py-3 text-slate-600 max-w-48 truncate">{r.contrato.direccion}</td>
                                        <td className="px-4 py-3 text-slate-600 max-w-48 truncate">{r.corte.motivo}</td>
                                        <td className="px-4 py-3">
